...
function TopicDetail(props) {
        ...
        const getCats = async () => {
            try {
                // isLoading(true)
                let response
                if (location.pathname.split('/')[1] === 'topics') {
                    response = await axiosInstance.get(`${baseUrl}api/category/${id}`);
                } else {
                    response = await axiosInstance.get(`${baseUrl}api/post/${id}`);
                }
            ...
            <div className='container'>
                ...
                <div className='w-8/12 mx-auto'>
                    <div className='flex items-center my-10 gap-3'>
                        <NavLink to={`/`} className=' text-[#CFCFCF] text-nowrap cursor-pointer'>Baş sahypa</NavLink>
                        <svg xmlns="http://www.w3.org/2000/svg" width="9" height="15" viewBox="0 0 9 15" fill="none">
                            <path d="M1 1L7 7.5L1 14" stroke="#CFCFCF" stroke-width="1.5" />
                        </svg>
                        <div className=' text-[#CFCFCF]'>{cats?.title}</div>
                    </div>

                    <div className=' flex items-center gap-5  right-5 left-0'>
                        <div className='wrap w-3/5 text-goldColor font-[500] text-[25px]'>{cats?.category_name}</div>
                        <Divider showright={true} className='mt-5' />

                    </div>
                    <div className={` ${cats?.category_name === cats?.title && 'hidden'} wrap w-4/5 text-goldColor mt-5 font-[500] text-[30px]`}>{cats?.title}</div>
                    ...
                    <div className=' mt-5 font-[500] innerHtml' dangerouslySetInnerHTML={{ __html: cats?.content }}>
                    ...
                    <div className={`mt-10 ${props.show ? '' : 'hidden'}`}>
                        <div className='font-[500] text-lg mt-20 mb-10'>Wakansiýa Barada Habarlaşmak üçin:</div>
                        <div className='flex flex-col gap-8'>

                            <div className='w-full  cursor-pointer  flex flex-col gap-2 mx-auto text-darkColor font-[500]'>
