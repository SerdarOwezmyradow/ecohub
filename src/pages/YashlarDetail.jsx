    ...
    return (
        <div className='container'>
            <div className='flex items-center my-10 gap-3'>
                ...
                <NavLink to={`/yaslar/`} className=' text-[#CFCFCF] cursor-pointer'>{cats && cats[0]?.category_category_name}</NavLink>
                {id && cats &&
            ...
            <div className='w-1/2  mx-auto'>
                ...
                <div className='w-full flex flex-col gap-3'>
                    {cats?.length > 0 && cats?.map((cat) => {
                        return (
                            <>
                                <NavLink to={`/topic/${cat.id}`} className=' bg-[#FAFAFA] w-full cursor-pointer font-[500] p-3  mx-auto text-darkColor'>{cat?.title} </NavLink>
                            </>
                        )
                    })}
                </div>
